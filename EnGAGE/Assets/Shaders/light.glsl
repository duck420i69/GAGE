...
vec3 CalSpecular(vec3 n, vec3 vec_to_light, vec3 light_color, float light_intensity, int specular_power, vec3 frag_pos_view) {
	return light_color * light_intensity *  pow( max(0.0f, dot(normalize(reflect(vec_to_light, n)), normalize(frag_pos_view))), specular_power);
}